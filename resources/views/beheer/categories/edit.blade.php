...
<form method="post" action="/beheer/categories/{{$category->id}}">
  ...
  <div class="form-group">
    ...
    <input type="text" class="form-control" id="categoryDescription" aria-describedby="categoryDescription" name="description" value="{{ $category->description}}" >
  </div>
  <div class="form-group mb-3">
     <input id="categoryActive" class="form-input" name="active" type="checkbox" value="{{ $category->id }}"
            @if($category->active == 1) checked=checked @endif
            >
      <label id="lblActive" class="form-check-label" for="categoryActive">Categorie Actief</label>
      <small><br><strong>Als vinkje uitstaat, Is de categorie uitgeschakeld.</strong></small>
  </div>
