...
<form method="post" action="/beheer/products/{{ $product->id}}">
  ...
  <div class="form-group">
    ...
    <input type="text" class="form-control" id="productDescription" aria-describedby="productDescription" name="productDescription"
  ...
  <div class="form-group mb-3">
    <input id="productActive" class="form-input" name="productActive" type="checkbox" value="{{ $product->active }}" @if($product->active == 1) checked=checked @endif >
    <label id="lblActive" class="form-check-label" for="productActive">Product Actief</label>
    <small>
      <br>
      <strong>Als vinkje uitstaat, staat product niet op de website</strong>
    </small>
  </div>
